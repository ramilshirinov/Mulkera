...
export default function RealtorsPage() {
  ...
  useEffect(() => {
    async function fetchRealtors() {
      setLoading(true);
      // Profilləri və onların satış/reytinq göstəricilərini çəkirik
      const { data, error } = await supabase
        .from("profiles")
        .select("*")
        .order("rating", { ascending: false }); // Reytinqə görə ilkin sıralama

      if (!error && data) {
        // Əgər bazada reytinq sütunu yoxdursa və ya simulyasiya lazımdırsa:
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="text-center max-w-2xl mx-auto mb-12">
        ...
        <p className="text-navy/70 text-sm sm:text-base">
          Satış sayı, müştəri məmnuniyyəti razılığı və satış sürətinə görə sıralanmış peşəkar rieltorlar arasından seçim edin.
        </p>
      </div>
